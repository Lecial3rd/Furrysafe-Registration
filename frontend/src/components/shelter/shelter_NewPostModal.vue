...
function handlePetSelected(info) {
 console.log("Selected Pet Info:", info); // Debugging line
  ...
  if (selectedPetInfo.value.length > 0) {
    const selectedPet = selectedPetInfo.value[0]; // Get the first selected pet
    post.pet_id = selectedPet.id; // Set pet_id from the selected pet
    ...
    post.pet_id = null; // Reset pet_id if no pet is selected
  }
}
...
async function submitPost() {
  ...
  console.log("Content:", newpost.value); // Ensure this is set correctly

  if (isLoading.value) return;

  // Check for required fields
  if (!post.user_id || !post.pet_id || !newpost.value) {
    console.error("Missing required fields: user_id, pet_id, or content.");
    return;
  }
  ...
  try {
    ...
    formData.append('content', newpost.value); // Use newpost here
    formData.append('lat', null); // Add latitude if available
    formData.append('long', null); // Add longitude if available
    formData.append('address', null); // Add address if available
    formData.append('pet_condition', null); // Add pet condition if available
    formData.append('photo_urls', JSON.stringify(imageUrls.value)); // Convert imageUrls to JSON string
